...
const MeasurementSection: React.FC<MeasurementSectionProps> = ({
  ...
  // State for default unit
  const [defaultUnit, setDefaultUnit] = useState(units[0] || "");

  // Optional: Update all measurements to use the default unit when it changes
  const handleDefaultUnitChange = (value: string) => {
    setDefaultUnit(value);
    // If you want the default unit to update all areacollection units
    const updatedSelections = selections.map((selection) => ({
      ...selection,
      areacollection: selection.areacollection.map((collection: any) => ({
        ...collection,
        measurement: { ...collection.measurement, unit: value },
      })),
    }));
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
        <div className="flex flex-col border rounded-lg w-full sm:w-1/4">
          ...
          {selections.map((selection, index) => (
            <div key={index} className="flex gap-2 items-center pl-4 py-2">
              <p className="text-sm sm:text-base">{selection.area}</p>
              <button
                className={`${selection.area === "" ? "hidden" : ""} mb-3 text-red-500 hover:text-red-700`}
            ...
            {selections.map((selection, mainindex) => (
              <div key={mainindex} className="flex flex-col gap-2 w-full">
                ...
                <div className="flex flex-col gap-4 w-full">
                  {selection.areacollection.map((collection: any, index: number) => (
                    <div
                      key={index}
                      className="flex flex-col sm:flex-row p-3 border rounded-lg md:gap-3 sm:gap-4 w-full justify-between"
                    >
                      <div className="flex flex-row w-full sm:gap-4 md:gap-2">
                        ...
                        <div className="flex flex-col gap-2 w-full sm:w-1/5">
                          ...
                          <select
                            ...
                            className="border border-gray-300 p-2 rounded-lg w-full text-sm sm:text-base focus:ring-2 focus:ring-blue-400"
                            value={collection.measurement.unit || defaultUnit} // Fallback to defaultUnit
                          >
                            {units.map((unit) => (
                              <option key={unit} value={unit}>
                                {unit}
                      ...
                      <div className="flex flex-col gap-2 sm:w-1/6">
                        {/* Add delete button or other controls here if needed */}
                      </div>
                    </div>
                  ))}
                </div>
