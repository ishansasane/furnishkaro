...
// Placeholder for fetchInquiryData since it was undefined
const fetchInquiryData = async () => {
  try {
    const response = await fetch(
      "https://sheeladecor.netlify.app/.netlify/functions/server/getinquiries",
      {
        credentials: "include",
      }
    );
...
const Dashboard: React.FC = () => {
  ...
  const reduxInquiries = useSelector((state: RootState) => state.data.inquiry);
  ...
  useEffect(() => {
    if (reduxInquiries.length === 0) {
      const fetchData = async () => {
        try {
          const data = await fetchInquiryData();
          dispatch(setInquiries(data)); // Assuming setInquiries is defined in dataSlice.ts
        } catch (err) {
  ...
  const handleMarkAsCompleted = async (status: string, name: string) => {
    try {
      const response = await fetch(
  ...
  const openProject = (selectedTask: any) => {
  ...
  const handleMarkAsApproved = (inquiry: any) => {
    setInquiries(
      inquiries.map((i) =>
        i.project === inquiry.project && i.inquiryDate === inquiry.inquiryDate
          ? { ...i, status: "Approved" }
            ...
            <div className="bg-white rounded-lg shadow-md p-6 w-full max-w-md relative border border-gray-200">
              ...
              <div className="flex justify-between mt-6">
                ...
                <button
                  ...
                  className={`${
                    selectedInquiry.status === "Convert to Project"
                      ? "hidden"
                      : ""
