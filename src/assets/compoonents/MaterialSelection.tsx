...
const MaterialSelection = ({ setMaterials }) => {
  ...
  const availableProductGroups = ["Tiles", "Wood", "Marble", "Paint", "Wallpaper"];
  ...
  const handleProductGroupChange = (index: number, product: string) => {
    ...
    if (selectedProductGroups.includes(product)) {
      updatedSelections[index].productGroups = selectedProductGroups.filter((p) => p !== product);
    } else {
      updatedSelections[index].productGroups = [...selectedProductGroups, product];
    }
  ...
  return (
    <div className="mb-6 bg-white p-6 rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-6 text-gray-700">Material Selection</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Left Column: Area Selection */}
        <div>
          <h3 className="text-lg font-semibold mb-4 text-gray-600">Select Areas</h3>
